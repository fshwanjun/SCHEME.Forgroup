...
import { TEST2_DISTORT_CONFIG } from '@/config/distortConfig';
...
export default function Test2() {
  ...
  useEffect(() => {
    ...
    if (mode === 'cover' && selectedProjectData) {
      ...
      // URL을 원래대로 복원
      window.history.replaceState({ zoomed: false }, '', '/test2');
    }
    ...
    const handlePopState = (e: PopStateEvent) => {
      ...
      if (currentMode === 'cover' || currentMode === 'center') {
        ...
        if (state?.modal && state?.step === 1) {
          ...
          // 히스토리 복원 후 줌아웃
          window.history.pushState({ modal: false }, '', '/test2');
          zoomOut();
        }
  ...
  return (
    <>
      ...
      <div
        ...
        className={cn(
          'h-svh overflow-x-hidden overscroll-none',
