...
import { TEST1_DISTORT_CONFIG } from '@/config/distortConfig';
...
export default function Test1() {
  ...
  useEffect(() => {
    ...
    if (prevMode === 'default' && (mode === 'center' || mode === 'cover')) {
      ...
      const newUrl = projectSlug ? `/projects/${projectSlug}` : '/test1';
      window.history.pushState({ modal: true, step: 1, originalPath: '/test1' }, '', '/test1');
      window.history.pushState({ modal: true, step: 2, originalPath: '/test1' }, '', newUrl);
    ...
    if (mode === 'cover' && selectedProjectData) {
      ...
      // URL을 원래대로 복원
      window.history.replaceState({ zoomed: false }, '', '/test1');
    }
      ...
      renderSections((id, index, isTrigger) => (
        <div key={id} ref={isTrigger ? setTriggerElement : null} data-section-id={id} data-is-trigger={isTrigger}>
          <HomeGallery
            ...
            sectionId={id}
            distortionScale={TEST1_DISTORT_CONFIG.distortionScale}
            radiusPercent={TEST1_DISTORT_CONFIG.radiusPercent}
            blurStd={TEST1_DISTORT_CONFIG.blurStd}
            easingFactor={TEST1_DISTORT_CONFIG.easingFactor}
          />
        </div>
      )),
    ...
    const handlePopState = (e: PopStateEvent) => {
      ...
      if (currentMode === 'cover' || currentMode === 'center') {
        ...
        if (state?.modal && state?.step === 1) {
          ...
          // 히스토리 복원 후 줌아웃
          window.history.pushState({ modal: false }, '', '/test1');
          zoomOut();
        }
